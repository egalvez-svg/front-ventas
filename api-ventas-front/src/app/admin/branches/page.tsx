...
import { Building2, Plus, Pencil, PowerOff, X, Loader2, ArrowLeft, FolderOpen, MapPin, CheckCircle, XCircle, Search } from "lucide-react";
import {
  useBranches,
  useCreateBranch,
  useUpdateBranch,
  ...
  return (
    <div className="min-h-full text-stone-900 dark:text-slate-50 p-6">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          onClick={openCreate}
          className="flex items-center gap-2 bg-amber-500 hover:bg-amber-400 text-white font-bold px-3 sm:px-4 py-2.5 rounded-xl transition-all active:scale-95 shadow-lg shadow-amber-500/20"
        >
      ...
      <div className="bg-white dark:bg-slate-900 border border-stone-200 dark:border-slate-800 rounded-2xl overflow-x-auto">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-3 text-stone-400 dark:text-slate-500">
            ...
            <p>{search ? "Sin resultados" : "No hay sucursales registradas"}</p>
          </div>
        ) : (
