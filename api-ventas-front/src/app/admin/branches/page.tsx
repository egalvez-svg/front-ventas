...
function BranchesContent() {
  ...
  const isPending =
    createBranch.isPending || updateBranch.isPending || deleteBranch.isPending;
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50 p-6">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          onClick={openCreate}
          className="flex items-center gap-2 bg-gradient-to-r from-sky-500 to-blue-500 hover:from-sky-400 hover:to-blue-400 text-white font-bold px-4 py-2.5 rounded-xl transition-all active:scale-95 shadow-lg shadow-sky-500/20"
        >
            ...
            <tbody className="divide-y divide-slate-800/50">
              {branches.map((branch) => (
                <tr key={branch.id} className="hover:bg-slate-800/30 transition-colors">
                  <td className="px-6 py-4 font-medium text-slate-200">{branch.name}</td>
                  ...
                  <td className="px-6 py-4">
                    {branch.is_active ? (
                      ...
                      <span className="inline-flex items-center gap-1.5 text-xs font-medium text-slate-500 bg-slate-700/40 px-2.5 py-1 rounded-full">
      ...
      {modal.mode === "delete" && (
        <ModalOverlay onClose={close}>
          <div className="text-center">
            ...
            <h2 className="text-xl font-bold text-white mb-2">¿Desactivar sucursal?</h2>
            <p className="text-slate-400 text-sm mb-6">
              Se desactivará{" "}
              <span className="text-white font-semibold">{modal.item.name}</span>.
              Podrá reactivarla editándola.
            </p>
  ...
  return (
    <div className="flex gap-3 pt-2">
      ...
      <button
        type="submit"
        disabled={disabled}
        className="flex-1 py-3 rounded-xl bg-gradient-to-r from-sky-500 to-blue-500 hover:from-sky-400 hover:to-blue-400 text-white font-bold transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
      >
...
const inputClass =
  "w-full px-4 py-3 bg-slate-800/60 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-sky-500/50 focus:border-sky-500 transition-all text-sm";
