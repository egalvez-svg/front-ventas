...
const STATUS_CONFIG: Record<OrderStatus, { label: string; classes: string }> = {
  pending:   { label: "Pendiente",   classes: "text-amber-400 bg-amber-500/10 border-amber-500/30" },
  cooking:   { label: "En cocina",   classes: "text-orange-400 bg-orange-500/10 border-orange-500/30" },
  served:    { label: "Servido",     classes: "text-sky-400 bg-sky-500/10 border-sky-500/30" },
  paid:      { label: "Pagado",      classes: "text-emerald-400 bg-emerald-500/10 border-emerald-500/30" },
  cancelled: { label: "Cancelado",   classes: "text-slate-500 bg-slate-500/10 border-slate-500/30" },
};
...
function OrdersContent() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50 p-6">
      ...
      {branchId === null ? (
        <div className="bg-slate-900 border border-slate-700 rounded-2xl p-8 max-w-sm">
          <p className="text-sm text-slate-400">Tu cuenta no tiene una sucursal asignada.</p>
        </div>
      ) : (
        <>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`inline-flex px-2.5 py-1 rounded-lg border text-xs font-semibold ${cfg.classes}`}
                          >
                            {cfg.label}
                          </span>
                        </td>
      ...
      {selected && (
        <ModalOverlay onClose={() => setSelected(null)}>
          ...
          <div className="mb-5">
            <span
              className={`inline-flex px-3 py-1.5 rounded-xl border text-sm font-semibold ${
          ...
          <div className="space-y-2 mb-6">
            ...
            {selected.items.map((item, i) => (
              <div
                key={i}
                className="bg-slate-800/50 rounded-xl px-4 py-3 flex items-start justify-between gap-4"
              >
                <div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div
        ...
        onClick={onClose}
      />
      <div className="relative w-full max-w-md bg-slate-900 border border-slate-700 rounded-3xl shadow-2xl p-8 max-h-[90vh] overflow-y-auto">
        {children}
      </div>
    </div>
  );
