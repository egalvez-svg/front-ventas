...
const ROLE_COLORS: Record<string, string> = {
  ...
  cashier: "text-cyan-400 bg-cyan-500/10 border-cyan-500/30",
  waiter: "text-emerald-400 bg-emerald-500/10 border-emerald-500/30",
  kitchen: "text-amber-400 bg-amber-500/10 border-amber-500/30",
};
...
const EMPTY_MEMBERSHIP: MembershipPayload = { branch_id: null, role: 'waiter' };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-50 p-6">
      <AppHeader
        ...
        actions={
          <button
            onClick={openCreate}
            className="flex items-center gap-2 bg-gradient-to-r from-emerald-500 to-cyan-500 hover:from-emerald-400 hover:to-cyan-400 text-slate-950 font-bold px-3 py-1.5 rounded-xl transition-all active:scale-95 text-sm shadow-lg shadow-emerald-500/20"
          >
                  ...
                  <tr key={user.id} className="hover:bg-slate-800/30 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center flex-shrink-0 text-emerald-400 font-bold">
                        ...
                        {user.memberships.map((m) => (
                          <div 
                            ...
                            className={`flex flex-col gap-0.5 px-2 py-1 rounded-lg border text-[10px] font-bold uppercase tracking-tighter ${
                              ROLE_COLORS[m.role] ?? "text-slate-400 bg-slate-700/50 border-slate-600"
                            }`}
                          >
      ...
      {(modal.mode === 'create' || modal.mode === 'edit') && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className={`relative w-full ${modal.mode === 'edit' ? 'max-w-xl' : 'max-w-md'} bg-slate-900 border border-slate-700 rounded-3xl shadow-2xl p-8 max-h-[90vh] overflow-y-auto`}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {modal.mode === 'create' && (
                <>
                  <div className="space-y-1.5">
                    <label className="text-xs font-medium text-slate-400 uppercase tracking-wider">Rol Inicial</label>
                    <select 
                      value={form.role}
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-medium text-slate-400 uppercase tracking-wider">Sucursal (opcional)</label>
                    <select
              ...
              {modal.mode === 'edit' && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-medium text-slate-400 uppercase tracking-wider">Roles y Sucursales</label>
                    <button
                      type="button"
                  ...
                  {memberships.length === 0 && (
                    <p className="text-slate-600 text-xs italic py-2">Sin membresías asignadas</p>
                  )}
                  {memberships.map((m, i) => (
                    <div key={i} className="flex items-center gap-2">
                      <select
                        ...
                        className="flex-1 px-3 py-2 bg-slate-800 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500"
                      >
                        {ROLES.map(r => (
                          <option key={r.value} value={r.value}>{r.label}</option>
                        ))}
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={isPending}
                  className="flex-1 py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 hover:from-emerald-400 hover:to-cyan-400 text-slate-950 font-bold transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
                >
      ...
      {modal.mode === 'delete' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-slate-900 border border-slate-700 rounded-3xl shadow-2xl p-8 text-center">
            ...
            <h2 className="text-xl font-bold mb-2">¿Desactivar a {modal.user.full_name}?</h2>
            <p className="text-slate-400 text-sm mb-8">
              El usuario ya no podrá iniciar sesión. Esta acción no se puede deshacer fácilmente.
            </p>
            <div className="flex gap-3">
              <button onClick={close} disabled={isPending} className="flex-1 py-3 border border-slate-700 rounded-xl text-slate-400 hover:bg-slate-800 transition-colors">
                Cancelar
              </button>
              <button 
...
const inputClass =
  "w-full px-4 py-3 bg-slate-800/60 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition-all text-sm";
